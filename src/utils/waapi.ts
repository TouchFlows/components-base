...
import { EASING_FUNCTIONS, EasingType } from '../easing/easing'
import { IAnimate } from '../models/animatable'
 ...
 */
function createAnimation(element: HTMLElement, context: IAnimate): Animation {
  const newKeyFrames =
    context.keyFrames ||
 ...
 */
export function clearPropsWithOptions(context: IAnimate, options: KeyframeAnimationOptions) {
 ...
 */
export function getAnimationOptions(context: IAnimate): KeyframeAnimationOptions {
  const animationOptions: KeyframeAnimationOptions =
    context.options || (context.optionsData && JSON.parse(context.optionsData)) || {}
  ...
  if (context.animateId !== undefined) animationOptions.id = context.animateId
...
export class AnimationManager {
  ...
  private state: IAnimate
  private animation: Animation = null
  private className: string
  private isUpdatingState: boolean

  constructor(initState: IAnimate) {
  ...
  loadAnimation() {
     ...
     * Add listeners
     */
    newAnimation.addEventListener('stop', this.onStopAnimation)
  ...
  clearAnimation() {
    ...
    this.animation.removeEventListener('stop', this.onStopAnimation)
   ...
   */
  playAnimation() {
    if (this.currentAnimation.playState === 'running' && !this.isUpdatingState) return
    /**
  ...
  setState(element: HTMLElement, newState: IAnimate) {
   ...
   */
  onRunAnimation = () => {
    this.state.onRun.emit(this.element)
  ...
  onCancelAnimation = () => {
    ...
    if (this.state.fromClassName !== undefined && this.className !== undefined) {
      this.element.className = this.className
    }
  }

  /**
   * Emit `onStop` event and update class name with `toClassName`.
   */
  onStopAnimation = () => {
    const { element, state } = this
    state.onStop.emit(element)
